...
export default function Maintenance({ route, navigation }) {
  ...
  const LoadInsight = async () => {
    ...
    try 
    {
      const url = AppConfig.webserver + "server/api.php?mode=maintenanceview";
          ...
          isLoading ? ( 
            ...
            <ScrollView style={{ flex: 1, }} contentContainerStyle={{ flexGrow: 1, padding: 20 }}>
                <View style={{ flex: 1, justifyContent: "flex-start" }}>
                  <Text style={{ color: AppConfig.whiteColor, fontSize: 30, fontWeight: 'bold', marginTop: 20, marginBottom: 10 }}>Maintenance</Text>
                  <Text style={{ color: AppConfig.whiteColor, fontSize: 20, fontWeight: "100", width: "75%" }}>There are some things that requires your attention</Text>
                  
                  <Surface style={styles.surface} elevation={4}>
                    <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                      <Text style={{ color: AppConfig.blackColor, fontSize: 25, fontWeight: "bold", verticalAlign: "middle" }}>Battery Percentage</Text>
                      <Image source={treeData.batteryicon + "?v=" + new Date()} style={{ alignSelf: 'center', height: 100, width: 100, marginTop: 15, }}></Image>
                    </View>
                  </Surface>
                </View>
            </ScrollView>
